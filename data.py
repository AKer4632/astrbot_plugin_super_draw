...
class PluginData:
    ...
    def __init__(self, config: AstrBotConfig, dataDir: Path):
        ...
        self.proxy: str | None = None  # HTTP 代理
        self.timeout: int = 180  # 请求超时秒数
        self.maxRetry: int = 3  # 最大重试次数
        self.maxConcurrent: int = 3  # 最大并发任务数

        # ─── 生图默认值 ───
        self.defaultSize: str = "auto"  # 映射后的 OpenAI size 值
        self.defaultQuality: str = "low"  # 映射后的 OpenAI quality 值
        self.showInfo: bool = False  # 生图后是否显示耗时等信息
        self.showModel: bool = False  # 生图后是否显示模型名
        self.maxImageSizeMB: int = 10  # 参考图最大 MB
    ...
    def mapResolution(resolution: str) -> str:
        ...
        return qualityMap.get(resolution, "auto")
    ...
    def _loadConfig(self) -> None:
        ...
        self.proxy = (api.get("proxy") or "").strip() or None

        # 生图配置
        gen = self.rawConfig.get("generation", {})
        self.timeout = gen.get("timeout", 180)
        self.maxRetry = gen.get("max_retry_attempts", 3)
        self.maxConcurrent = max(1, gen.get("max_concurrent_tasks", 3))
        self.showInfo = gen.get("show_generation_info", False)
        self.showModel = gen.get("show_model_info", False)

        # 把配置里的中文比例和分辨率映射成 OpenAI 参数
        rawRatio = gen.get("default_aspect_ratio", "自动")
        rawResolution = gen.get("default_resolution", "1K")
        self.defaultSize = self.mapAspectRatio(rawRatio)
        ...
        self.maxImageSizeMB = max(1, limits.get("max_image_size_mb", 10))

        # 缓存配置
        cache = self.rawConfig.get("cache", {})
        self.maxCacheCount = max(1, cache.get("max_cache_count", 100))
        self.cleanupIntervalHours = max(1, cache.get("cleanup_interval_hours", 24))
